...
OUTPUT_DIR = Path("serpro/paineis_por_grupo_v4_projecao")
...
def build_group_panel(group_name: str, rows: list[dict[str, str]], output_file: Path) -> None:
    ...
    ax_main.set_ylabel("Percentual (%)", fontsize=16)
    ax_main.set_xticks(x_positions)
    ax_main.set_xticklabels(labels, fontsize=13)
    ...
    # Linha de apoio com texto de questoes resumidas (quebrada para evitar corte).
    ax_main.text(
        0.005,
        -0.115,
        compact_question_map(rows),
    ...
    ax_var.set_yticks(y)
    ax_var.set_yticklabels(names, fontsize=15)
    ax_var.tick_params(axis="x", labelsize=13)
    ...
    for yi, v in zip(y, vals):
        ...
        arrow = "▲" if v >= 0 else "▼"
        ax_var.text(v + dx, yi, f"{arrow} {v:+.2f} pp", va="center", ha=ha, fontsize=15, color="#1d2939")

    # Sintese analitica por palavras-chave.
    ax_key.set_xticks([])
    ax_key.set_yticks([])

    # Seleciona ate 4 perguntas com maior mudanca em concorda.
    ...
    lines = wrap_lines(lines, width=58)
    ...
    # Painel extra: legenda bonita das perguntas
    ax_q.set_xticks([])
    ax_q.set_yticks([])
    ax_q.set_xlim(0, 1)
    ax_q.set_ylim(0, 1)
    ...
    # Distribui as perguntas com boxes largos (3 colunas fixas) e espaçamento generoso para telão.
    n_cols = 3
    n_rows = int(np.ceil(n_questions / n_cols))
    dx = 1 / n_cols
    dy = 1 / (n_rows + 0.15) if n_rows else 1
    ...
    for i, row in enumerate(rows):
        ...
        x0 = col * dx + 0.025
        y0 = 0.96 - r * dy
        wrapped = "\n".join(wrap_lines([f"Q{i + 1}: {row['Perguntas']}"] , width=32))
        ax_q.text(
            x0,
            y0,
            wrapped,
...
def main() -> None:
    ...
    for group_name, group_rows in groups.items():
        out = OUTPUT_DIR / f"painel_{slugify(group_name)}_v4_projecao.png"
